...
class GxAircom(Provider):
    ...
    def process_data(self):
        ...
        data = requests.get("http://www.mgenet.at/gxaircom/stations.php", timeout=(self.connect_timeout, self.read_timeout)).json()
        for station in data:
            winds_station = self.save_station(
                provider_id=station["stationId"],
                short_name=station["stationName"],
                name=station["stationName"],  # Lets winds.mobi provide the full name with the help of Google Geocoding API
            ...
            if not self.has_measure(measures_collection, measure_key):
                new_measure = self.create_measure(
                    ...
                    pressure=Pressure(station["pressure"], qnh=None, qff=None) if station["pressure"] is not None else None,
                )
